...
export class AuthService {
  ...
  // ── Resolve tenant from slug or id ───────────────────────────────────
  private async resolveTenant(slug?: string, tenantId?: string) {
    if (!slug && !tenantId) {
      throw new UnauthorizedException('Se requiere slug de empresa o tenantId');
    }
    const tenant = slug
  ...
  async login(dto: LoginDto) {
    ...
    return {
      access_token: this.issueToken(user.id, user.email, tenant.id, permissions),
      user: {
        id: user.id,
        email: user.email,
  ...
  async registerTenant(dto: RegisterTenantDto) {
    ...
    if (slugTaken) {
      throw new ConflictException(`El slug "${dto.tenant.slug}" ya está en uso`);
    }
    ...
    return {
      access_token: this.issueToken(
        result.user.id, result.user.email, result.tenant.id, result.permissions,
      ),
      user: {
        id: result.user.id,
  ...
  // ── Internal ──────────────────────────────────────────────────────────
  private issueToken(userId: string, email: string, tenantId: string, permissions: string[]): string {
